<script>
	export default {
		onLaunch: function() {
			...
			if (updated.completed === true) {
				...
				plus.runtime.getProperty(plus.runtime.appid, widgetInfo => {
					//请求后台接口 解析数据 对比版本
					this.$Request.get('/app/appinfo/list').then(res => {
						console.log(res.data[0], '222222222')
						res = res.data[0];
			...
			// #ifdef H5
			let invitation = this.$queue.getInvitation();
			if (window.location.href.indexOf('?invitation=') !== -1 || window.location.href.indexOf('&invitation=') !==
				...
				if (window.location.href.indexOf('?invitation=') !== -1) {
					...
					this.$queue.setData('invitation', window.location.href.split('?invitation=')[1].split('&')[0]);
				} else {
					invitation = window.location.href.split('&invitation=')[1].split('&')[0];
					this.$queue.setData('invitation', window.location.href.split('&invitation=')[1].split('&')[0]);
				}
			...
			if (ua.indexOf('micromessenger') !== -1) {
				...
				if (!openid) {
					if (window.location.href.indexOf('?code=') !== -1 || window.location.href.indexOf('&code=') !== -
						...
						this.$Request.getT('/app/Login/openId/' + code).then(res => {
							if (res && res != '') {
								this.$queue.setData('openId', res.data);
								let openid = res;
								this.$Request.getT('/app/Login/openIdLogin?wxId=' + res.data + '&invitation=' +
									invitation).then(rese => {
									if (rese.code == 0) {
										this.$queue.setData('token', rese.token)
										this.$queue.setData('userName', rese.user.userName)
										this.$queue.setData('avatar', rese.user.avatar)
										this.$queue.setData('phone', rese.user.phone)
										this.$queue.setData('invitationCode', rese.user
											.inviterCode)
										this.$queue.setData('sex', rese.user.sex)
										this.$queue.setData('userId', rese.user.userId)
										this.$queue.setData('openId', rese.user.wxId)
										setTimeout(() => {
											this.$router.go(0)
										}, 400)
									}
					...
					this.$Request.getT('/app/Login/openIdLogin?wxId=' + openid + '&invitation=' + invitation).then(
						res => {
							if (res.code == 0) {
								...
								this.$queue.setData('invitationCode', res.user.inviterCode)
								this.$queue.setData('sex', res.user.sex)
								this.$queue.setData('userId', res.user.userId)
								this.$queue.setData('openId', res.user.wxId)
							}
						});
				}
			}
		...
		onShow: function() {
			...
			this.$Request.get('/app/common/type/618').then(res => { //客服配置  1  手机号 2 微信公众号客服  3微信
				if (res.code == 0 && res.data) {
					uni.setStorageSync('kefu', res.data.value)
				}
			});
			this.$Request.get('/app/common/type/619').then(res => { //客服电话 
				if (res.code == 0 && res.data) {
					uni.setStorageSync('kefuPhone', res.data.value)
				}
			});
			this.$Request.get('/app/common/type/616').then(res => { //微信客服链接 
				if (res.code == 0 && res.data) {
					uni.setStorageSync('kefuUrl', res.data.value)
				}
			});
			this.$Request.get('/app/common/type/617').then(res => { //微信客服appid   
				if (res.code == 0 && res.data) {
					uni.setStorageSync('kefuAppId', res.data.value)
				}
			});
			// 激励广告id	622
			this.$Request.get('/app/common/type/622').then(res => {
				if (res.code == 0 && res.data) {
					uni.setStorageSync('adUnitId', res.data.value)
				}
			});
			// 抖音激励广告id	821
			this.$Request.get('/app/common/type/821').then(res => {
				if (res.code == 0 && res.data) {
					uni.setStorageSync('dyadUnitId', res.data.value)
				}
			});
			// 是否开启看广告	623
			this.$Request.get('/app/common/type/623').then(res => {
				if (res.code == 0) {
			...
			// 免费游玩次数	624
			this.$Request.get('/app/common/type/624').then(res => {
				if (res.code == 0) {
					if (res.data && res.data.value) {
						uni.setStorageSync('mfNum', res.data.value)
					}
				}
			});
			// 抖音企业号	825
			this.$Request.get('/app/common/type/825').then(res => {
				if (res.code == 0) {
					if (res.data && res.data.value) {
						uni.setStorageSync('awemeId', res.data.value)
					}
				}
			});
			// 是否开启支付	828
			this.$Request.get('/app/common/type/828').then(res => {
				if (res.code == 0) {
					if (res.data && res.data.value) {
						uni.setStorageSync('showPlay', res.data.value)
					}
				}
			});
			// #ifdef H5
			...
			// if (ua.indexOf('micromessenger') === -1) {
			// 	if (window.location.href.indexOf('/my/follow/follow') === -1) {
			// 		uni.reLaunch({
