<template>
	<view style="background: #FFFFFF;width: 100%;">
		...
		<image v-else lazy-load :show-menu-by-longpress="true" :src="tempFilePath" mode="widthFix" class="is-response"
	...
	export default {
		...
		methods: {
			...
			async OnCanvas() {
				...
				_this.ctx.setFillStyle('none'); //canvas背景颜色
				...
				let _strHeight = r[1] + C_P; //绘制字体距离canvas顶部的初始高度
				let _num = 1;
				...
				_this.ctx.drawImage(_QrCode.path, r[0] - q[0] + C_P - 160, _strHeight - 180, q[0] + 40, q[1] + 40);
				//添加二维码 end
