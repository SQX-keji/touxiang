<template>
	<view class="container">
		...
		<view class="mp_wxBox">
			<view>
				...
				<button v-show="weixinPhone"
					style="background: linear-gradient(0deg, #579EFF 0%, #579EFF 100%);color: #FFFFFF;" class="bottom"
		...
		<view style="text-align: center;">
			<view class="footer flex align-center justify-center">
				<view class="flex align-center">
					<!-- <text>登录即代表同意</text> -->
					<text @tap="showAgree = !showAgree" class="cuIcon"
		...
		<view style="text-align: center;">
			...
			<view style="font-size: 28upx;margin-top: 32upx;color: #333333;">欢迎使用头像生成系统</view>
		...
		methods: {
					...
					success: function(loginRes) {
								...
								if (Number(res.data.flag) === 1) {
									...
									that.$Request.postJson('/app/Login/dyRegister', {
										...
										if (ress.code == 0) {
											...
											uni.setStorageSync('invitationCode', ress.user.invitationCode)
						...
						that.$Request.get('/app/Login/wxLogin', data).then(res => {
							if (res.code == 0) {
								...
								let inviterCode = '';
								if (uni.getStorageSync('invitation')) {
									inviterCode = uni.getStorageSync('invitation')
								}
								let sendData = {
			...
			getWeixinInfo(sendData) {
				...
				let postData = {
					...
					type: 2,
					// shopOpenId: sendData.shopOpenId
				};
